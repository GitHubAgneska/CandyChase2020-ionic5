...
describe( 'CandyApiService', () => {
  ...
  let ApiMock: HttpTestingController;
  ...
  beforeEach( async () => {
    ...
    ApiMock = TestBed.get(HttpTestingController);
  ...
  it('#getAllCandyFromApi should return expected candylist', fakeAsync(() => {

    const service: CandyApiService = TestBed.get(CandyApiService);
    const expectedMockRawDataFromApi = { page_size: 50, page: 1, count: '6882',
    products: [{ _id: '1', product_name: 'testName1' },
              { _id: '2', product_name: 'testName2' },
              { _id: '3', product_name: 'testName3' }]};
    const  expectedMockCandy: CandyI[] = expectedMockRawDataFromApi.products;

    // httpClientSpy.get.and.returnValue(of(expectedCandy));

    service.getAllCandyFromApi().subscribe((data) => { data = data;
      // expect(data).toBeDefined();
      // expect(data).toEqual(expectedMockCandy, 'expectedCandy'), fail(httpErrorhandler);
    });
    // expect(httpClientSpy.get.calls.count()).toBe(1, 'one call');

    // const req = ApiMock.expectOne(candyApiTestUrl);
    // expect(req.request.method).toEqual('GET');
    // req.flush(expectedMockCandy);
    }));
  });
